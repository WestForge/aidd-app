...
interface AiddCapabilitySummary {
  ...
  status?: string;
  components?: string[];
  roadmapHorizon?: AiddRoadmapHorizon;
  targetDate?: string;
  estimatedSize?: AiddRoadmapSize;
  estimateConfidence?: AiddRoadmapConfidence;
  riskLevel?: AiddRoadmapRiskLevel;
  reviewBurden?: AiddRoadmapReviewBurden;
...
type AiddRoadmapHorizon = 'now' | 'next' | 'later' | 'parking-lot';
type AiddRoadmapSize = 'tiny' | 'small' | 'medium' | 'large' | 'too-large';
type AiddRoadmapConfidence = 'low' | 'medium' | 'high';
type AiddRoadmapRiskLevel = 'low' | 'medium' | 'high';
type AiddRoadmapReviewBurden = 'low' | 'medium' | 'high';
type AiddRoadmapCourseStatus = 'on-course' | 'tight' | 'at-risk' | 'off-course' | 'unknown';

interface AiddEntityProvenance {
...
interface AiddRoadmapCapabilityProgress {
  totalChanges: number;
...
interface AiddRoadmapCapabilityCourse {
  status: AiddRoadmapCourseStatus;
  reasons: string[];
  suggestedActions: string[];
}

interface AiddRoadmapCapability {
  slug: string;
  title: string;
  status: string;
  components: string[];
  ...
  estimateReason: string[];
  unknowns: string[];
  provenance: AiddEntityProvenance;
  progress: AiddRoadmapCapabilityProgress;
  course: AiddRoadmapCapabilityCourse;
}

interface AiddRoadmapSizeBucket {
  size: AiddRoadmapSize;
  label: string;
  count: number;
}

interface AiddRoadmapPressureMonth {
  month: string;
  capabilityCount: number;
...
interface AiddRoadmapReport {
  generatedAt: string;
  overallStatus: AiddRoadmapCourseStatus;
  summary: {
    capabilityCount: number;
  ...
  suggestedActions: string[];
  sizeBuckets: AiddRoadmapSizeBucket[];
  pressureByTargetMonth: AiddRoadmapPressureMonth[];
